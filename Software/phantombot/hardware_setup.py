...
def setup_hardware():
    # I2C und OLED
    i2c = busio.I2C(scl=getattr(board, PIN_CONFIG["i2c_scl"]),
                    sda=getattr(board, PIN_CONFIG["i2c_sda"]))
    oled = SSD1306_I2C(DISPLAY_WIDTH, DISPLAY_HEIGHT, i2c)

    # UART für VISCA
    uart = busio.UART(tx=getattr(board, PIN_CONFIG["uart_tx"]),
    ...
    # Encoder
    encoder = rotaryio.IncrementalEncoder(getattr(board, PIN_CONFIG["encoder_a"]),
